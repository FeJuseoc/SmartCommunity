...
Manager::Manager(QWidget *parent) : BaseWindow(parent), ui(new Ui::Manager)
{
    ui->setupUi(this);
    ...
    ui->pushButtonBack->setIcon(QIcon(":/image/back1.png"));
    ...
    connect(ui->pushButtonMenu, &QPushButton::clicked, this, [this]() {
        ...
        ui->pushButtonBack->setVisible(!isVisible); // 控制返回按钮的显示与隐藏
    });
...
void Manager::on_queryComplaint_clicked()
{
    ...
    while (query.next()) {
     ...
     ui->tableView_complaint->resizeRowsToContents();
}
...
void Manager::on_pushButtonClose_clicked()
{
    close();
}

void Manager::on_pushButtonMinimize_clicked()
{
    setWindowState(Qt::WindowMinimized);
}

void Manager::on_pushButtonBack_clicked()
{
    this->hide(); // 隐藏当前窗口
