...
Owner::Owner(QWidget *parent) : BaseWindow(parent), ui(new Ui::Owner)
{
    ui->setupUi(this);

    // 主题切换按钮连接
    connect(ui->btn_1, &QPushButton::clicked, this, &BaseWindow::set_style);
    connect(ui->btn_2, &QPushButton::clicked, this, &BaseWindow::set_style);
    connect(ui->btn_3, &QPushButton::clicked, this, &BaseWindow::set_style);
    connect(ui->btn_4, &QPushButton::clicked, this, &BaseWindow::set_style);

    // 连接 pushButtonMenu 点击事件
    connect(ui->pushButtonMenu, &QPushButton::clicked, this, [this]() {
        ...
        ui->pushButtonBack->setVisible(!isVisible);
    });

    // 默认隐藏 btn_1, btn_2, btn_3, btn_4 和 pushButtonBack
    ...
    // 设置图标
    QIcon logoIcon("D:/qt/community_4/image/screen.png");
    ui->pushButtonLogo->setIcon(logoIcon);
    ui->pushButtonLogo->setIconSize(QSize(32, 32));

    QIcon menuIcon("D:/qt/community_4/image/menu-hover.png");
    ui->pushButtonMenu->setIcon(menuIcon);
    ui->pushButtonMenu->setIconSize(QSize(32, 32));

    QIcon minimizeIcon("D:/qt/community_4/image/minimize-hover.png");
    ui->pushButtonMinimize->setIcon(minimizeIcon);
    ui->pushButtonMinimize->setIconSize(QSize(32, 32));

    QIcon closeIcon("D:/qt/community_4/image/close-hover.png");
    ui->pushButtonClose->setIcon(closeIcon);
    ui->pushButtonClose->setIconSize(QSize(32, 32));

    QIcon backIcon("D:/qt/community_4/image/back1.png");
    ui->pushButtonBack->setIcon(backIcon);
    ui->pushButtonBack->setIconSize(QSize(32, 32));
...
void Owner::on_pushButtonClose_clicked()
{
    close();
}

void Owner::on_pushButtonMinimize_clicked()
{
    setWindowState(Qt::WindowMinimized);
}

void Owner::on_pushButtonBack_clicked()
{
    this->hide(); // 隐藏当前窗口
    emit backToLogin(); // 发出返回登录界面的信号
}
...
void Owner::showtableview_8(){
    ...
    if (a.exec() && b.exec()) {
        ...
        while (hasPropertyFee) {
            ...
            rowItems.append(new QStandardItem(a.value(1).toString())); // 物业费日期
...
void Owner::cost(){
    ...
    if(ui->comboBox_4->currentText() == "车位费"){
        ...
        a.prepare("UPDATE community_cost SET community_cost_status = ? WHERE name = ? AND phone = ? AND community_cost_status = ?");
        a.addBindValue("已缴费");
        a.addBindValue(ownername);
        a.addBindValue(ownerphone);
        a.addBindValue("未缴费"); // 只更新未缴费的记录
